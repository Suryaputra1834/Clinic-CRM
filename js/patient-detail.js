...
function createVisitCard(visitId, visit) {
    ...
    <div class="visit-actions">
        ...
        <button onclick="shareViaWhatsApp('${visitId}')" class="btn-whatsapp-small">📱 WhatsApp</button>
...
async function shareViaWhatsApp(visitId) {
    try {
        // Get visit data
        const visitDoc = await db.collection('visits').doc(visitId).get();
        ...
        // Format visit data for WhatsApp
        ...
        let message = `*${doctorInfo.clinicName || 'Medical Clinic'}*\n`;
        message += `Dr. ${doctorInfo.name || 'Doctor'}\n\n`;
        message += `*PRESCRIPTION*\n`;
        message += `━━━━━━━━━━━━━━━━━━━━\n\n`;
        message += `*Patient:* ${currentPatient.name}\n`;
        ...
        if (visit.medicines && visit.medicines.length > 0) {
            ...
            visit.medicines.forEach((med, index) => {
                ...
                if (med.duration) message += ` (${med.duration})`;
                message += `\n`;
            });
            message += `\n`;
        }
        ...
        if (visit.followUpRequired && visit.followUpDate) {
            ...
            message += `⏰ *Follow-up:* ${followUpStr}\n\n`;
        }
        
        message += `━━━━━━━━━━━━━━━━━━━━\n`;
        message += `_Generated by Clinic CRM_`;
        ...
        showSuccess('Opening WhatsApp... Select contact to share!');
        
    } catch (error) {
        console.error('Error sharing via WhatsApp:', error);
        showError('Error sharing prescription: ' + error.message);
    }
}
